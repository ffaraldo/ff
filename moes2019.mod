...
param Tminheating{UtilitiesOfType['Heating'],Time} default 0;

# reference flow of the heating and cooling [kW]
param Qheatingsupply{UtilitiesOfType['Heating']} default refSize;
...
param Fmin{Utilities} default 0;
param Fmax{Utilities} default 0;
...
#subject to size_cstr4{u in Utilities, t in Time}: 				# if the utility is used in one time step, you have to buy it
#	use_t[u, t] <= use[u];
...
subject to LT_balance{t in Time}:
	Qheatingdemand['LowT',t] = sum{u in UtilitiesOfType['Heating']: Tminheating[u,t] >= Theating['LowT'] + dTmin} (Qheatingsupply[u] * mult_heating_t[u,t,'LowT']);
subject to MT_balance{t in Time}:
	Qheatingdemand['MediumT',t] = sum{u in UtilitiesOfType['Heating']: Tminheating[u,t] >= Theating['MediumT'] + dTmin} (Qheatingsupply[u] * mult_heating_t[u,t,'MediumT']);
